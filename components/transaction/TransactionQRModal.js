...
const TransactionQRModal = ({ modalOpen, setModalOpen, userAddress, setQrCode }) => {
    const { transactions, setTransactions } = useTrustRent();
    ...
    useEffect(() => {
        ...
        let retryInterval = 10000; // Start with a 10 second delay
        const maxInterval = 100000; // Maximum interval of 100 seconds
        ...
        const interval = setInterval(async () => {
            ...
            try {
                ...
                handleConfirmedTransaction(signatureInfo, transactions, recipient, reference);
                setModalOpen(false);
                clearInterval(interval);
            } catch (e) {
                if (e instanceof FindReferenceError || e.code === 429) {
                    // Adjust retry interval on rate limit error or no transaction found yet
                    retryInterval = Math.min(retryInterval * 2, maxInterval);
                    console.log(`Adjusting retry interval to ${retryInterval}ms`);
                    return;
                }
    ...
    }, [userAddress, connection, transactions, setTransactions, setModalOpen]);
...
const handleConfirmedTransaction = (signatureInfo, transactions, recipient, reference) => {
